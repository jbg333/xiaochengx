 ...
 * modified by zhangsy
 */
function editOneSmallWindow(url,id){
    parent.layer.open({
        type: 2,
        title: '修改',
        shadeClose: false,
 ...
 * @param id  选中的id
 */
function infoOneSmallWindow(url,id){
    parent.layer.open({
        type: 2,
        title: '详情',
        shadeClose: false,
    ...
    if(ids!=null){
        confirm("确认"+msg+"？",function(){
            $.ajax({
                type: "post",//发送方式
                url: url,//请求路径
                contentType: "application/json",//解决post  乱码问题
                data: JSON.stringify(ids),//转换成json字符串     他的返回值：包含JSON文本的字符串
                async: false,//同步锁住浏览器
 ...
 * @param width
 * @param height
 * @param refresh  search 通过search-btn class 点击刷新     location 通过 reload 刷新
 */
function openDefineIframe(title,url,width,height,isSearchRefresh){
	var widthPx = "1000px";
	var heightPx = "700px";
	...
	parent.layer.open({
		...
		content: url,
			...
			if('search' == isSearchRefresh) {
				$(".search-btn").click();
			} else if('location' == isSearchRefresh){
				location.reload();
			}
		}
	});
...
function addTab(url,name,id){
    var tab=url+"/"+id;
	//判断该页面是否已存在
    if ($("#navTab", window.parent.document).find("li[data-url='" + tab + "']").length === 0) {
        var index = Loading.open(1,false);
        //如果不存在
        $("#navTab", window.parent.document).find("li").removeClass("selected");
        //新增tab页
        var _li = ['<li tabid="tools-utils" class="selected" data-url="'+tab+'">',
      ...
      //新增右侧更多list
        ...
        var moreli = '<li class="selected" data-url="'+tab+'"><a href="javascript:"  title="' + name + '">' + name + '</a></li>';
