...
(function ($) {
    ...
    /* modified by zhangsy */
    $.fn.getRowData = function (primary,key) {
    	var resultObj = {};
    ...
    var PageGrid = {
        ...
        getData: function (url) {
            defaultParam.nd = new Date().getTime();
            defaultParam._ = new Date().getTime();
            var data;
            $.ajax({
                url: url,
                async: false,
        ...
        setPage: function (R, $grid, url) {
            ...
            layui.use(['laypage', 'layer'], function () {
                ...
                laypage.render({
                    ...
                    , curr:  R.page ? R.page.currPage : 1
                    , layout: ['count', 'prev', 'page', 'next', 'limit', 'skip']
                    , jump: function (obj,first) {
                        var index=Loading.open(1,false);
                        //解决点击查询两次后台请求问题(增加first参数)
                        PageGrid.toPage(R,obj, $grid, url,first);
                        Loading.close(index);
                    }
                });
            });
                    ...
                    if (operate == "true") {
                        ...
                        for (var key in data[i]) {
                            if (key == _name) {
                                ...
                                var value = data[i][key]; // 0不显示故注释后续内容 || "";
                                //如果是主键并且主键不为空，设置主键值
                                if (isPrimary == "true") {
                                    primary = value;
                                }
                        ...
                        /* modified by zhangsy */
                        if($.trim(_name)!="" && isChecked == false){
                        	$(_tr).append('<td name=' + _name + '></td>');//默认空列
                        }
        ...
        toPage: function (R,obj, $grid, url,first) {
        ...
        searchForm: function () {
            layui.use(['form'], function () {
                ...
                form.on('submit(search)', function (data) {
                    ...
                    /* modified by zhangsy */
                    if(props.searchAfterFunction!=null){
                    	//查询回调函数
                    	props.searchAfterFunction();
                    }
                    return false;
                });
